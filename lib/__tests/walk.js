...
describe('AST traversal', () => {
    ...
    it('path tracking test', () => {
        const ast = parse('.a { color: red }');
        const pathLog = [];

        walk(ast, {
            enter: function(node) {
                const pathInfo = this.path.map(n => n.type);
                pathLog.push({
                    node: node.type,
                    path: [...pathInfo]
                });
        ...
        const expected = [
            { node: 'StyleSheet', path: ['StyleSheet'] },
            { node: 'Rule', path: ['StyleSheet', 'Rule'] },
            { node: 'SelectorList', path: ['StyleSheet', 'Rule', 'SelectorList'] },
            { node: 'Selector', path: ['StyleSheet', 'Rule', 'SelectorList', 'Selector'] },
            { node: 'ClassSelector', path: ['StyleSheet', 'Rule', 'SelectorList', 'Selector', 'ClassSelector'] },
            { node: 'Block', path: ['StyleSheet', 'Rule', 'Block'] },
            { node: 'Declaration', path: ['StyleSheet', 'Rule', 'Block', 'Declaration'] },
            { node: 'Value', path: ['StyleSheet', 'Rule', 'Block', 'Declaration', 'Value'] },
            { node: 'Identifier', path: ['StyleSheet', 'Rule', 'Block', 'Declaration', 'Value', 'Identifier'] }
        ];

        assert.deepStrictEqual(pathLog, expected);
    });
